...
func (w *TestWorkload) GetApplicationProfile() (*v1beta1.ApplicationProfile, error) {
	...
	for _, appProfile := range appProfiles {
		wlKind := appProfile.Labels["seclogic.io/workload-kind"]
		wlName := appProfile.Labels["seclogic.io/workload-name"]
		wlNs := appProfile.Labels["seclogic.io/workload-namespace"]
...
func (w *TestWorkload) GetNetworkNeighborhood() (*v1beta1.NetworkNeighborhood, error) {
	...
	for _, n := range nn {
		wlKind := n.Labels["seclogic.io/workload-kind"]
		wlName := n.Labels["seclogic.io/workload-name"]
		wlNs := n.Labels["seclogic.io/workload-namespace"]
...
func (w *TestWorkload) WaitForApplicationProfile(maxRetries uint64, expectedStatus string) error {
	return backoff.RetryNotify(func() error {
		...
		if appProfile.Annotations["seclogic.io/status"] == expectedStatus {
			return nil
		}
...
func (w *TestWorkload) WaitForNetworkNeighborhood(maxRetries uint64, expectedStatus string) error {
	return backoff.RetryNotify(func() error {
		...
		if networkNeighborhood.Annotations["seclogic.io/status"] == expectedStatus {
			return nil
		}
